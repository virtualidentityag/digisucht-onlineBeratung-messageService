...
public class RocketChatService {
  ...
  MessageStreamDTO obtainMessageStream(String rcToken, String rcUserId, URI uri) {
    ...
    try {
      ...
      var msg = String.format("HTTP error while retrieving message stream from Rocket.Chat API (uri: %s): %s",
          uri, ex.getMessage());
      ...
      var msg = String.format("RestClientException while accessing Rocket.Chat API (uri: %s): %s",
          uri, ex.getMessage());
      LogService.logRocketChatServiceError(ex);
      throw new InternalServerErrorException(msg, LogService::logRocketChatServiceError);
    } catch (Exception ex) {
      var msg = String.format("Unexpected error while retrieving message stream from Rocket.Chat API (uri: %s): %s",
          uri, ex.getMessage());
      LogService.logRocketChatServiceError(ex);
      throw new InternalServerErrorException(msg, LogService::logRocketChatServiceError);
    }
